...
import { Plus, Edit2, Trash2, Trophy, X, Search } from "lucide-react";
import { supabase } from "@/lib/supabase";
import toast from "react-hot-toast";

const MOCK_ACHIEVEMENTS = [
  { id: "1", title: "Smart India Hackathon", description: "Winner", date: "2023", badge: "", link: "" },
];
...
export default function AchievementsAdmin() {
  ...
  const handleOpenModal = (ach?: any) => {
    if (ach) {
      ...
      setEditingAch(null);
      setFormData({ title: "", description: "", date: "", badge: "", link: "" });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading(editingAch ? "Updating achievement..." : "Adding achievement...");

    try {
      let error;
      if (editingAch) {
        const { error: updateError } = await supabase.from("achievements").update(formData).eq("id", editingAch.id);
        error = updateError;
      } else {
        const { error: insertError } = await supabase.from("achievements").insert([formData]);
        error = insertError;
      }

      if (error) throw error;
      toast.success(editingAch ? "Achievement updated!" : "Achievement added!", { id: toastId });
  ...
  const handleDelete = async () => {
    ...
    try {
      const { error } = await supabase.from("achievements").delete().eq("id", deletingId);
      if (error) throw error;
          ...
          {filteredAchievements.map((ach) => (
            <div key={ach.id} className="glass-panel p-6 rounded-2xl border border-white/10 group hover:border-accent-purple/50 transition-all flex justify-between items-center">
              ...
              <div className="flex gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                <button onClick={() => handleOpenModal(ach)} className="p-2 rounded-lg hover:bg-white/10 text-accent-lavender transition-colors" title="Edit">
                  <Edit2 className="w-4 h-4" />
                </button>
                <button onClick={() => confirmDelete(ach.id)} className="p-2 rounded-lg hover:bg-red-500/10 hover:text-red-400 text-gray-500 transition-colors" title="Delete">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#111] border border-white/10 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/10 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white">{editingAch ? "Edit Achievement" : "Add Achievement"}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-300">Achievement Title</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-300">Description (e.g. Winner, Finalist)</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-300">Date/Year</label>
                  <input 
                    type="text" 
                    value={formData.date}
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-300">External Link</label>
                  <input 
                    type="url" 
                    value={formData.link}
